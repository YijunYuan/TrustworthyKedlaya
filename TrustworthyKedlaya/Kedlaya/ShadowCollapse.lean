...
public import TrustworthyKedlaya.Kedlaya.Coeffwise
...
`TrustworthyKedlaya.Kedlaya.EngineTruncUP`.

The proof works at the lifted level: at each exponent the Teichmüller carry
congruence (`TrustworthyKedlaya.Kedlaya.TeichCarry`) makes the defect coefficientwise
