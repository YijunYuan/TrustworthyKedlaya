...
`ℚᵘⁿ_[p]`: it is a root of `X^d - p^{num}·[c^d]` where `d = q.den`, because
...
/-- **One-term series are integral over `ℚᵘⁿ_[p]`**: the series `[c]·p^q` is a root
of the monic polynomial `X^d - p^{num}·[c^d]` with `d = q.den`, `num = q.num`. -/
theorem single_mem_integralClosure_QpUn (q : ℚ) (c : 𝔽ᵃ_[p]) :
    single (p := p) q c ∈ integralClosure ℚᵘⁿ_[p] 𝕃_[p] := by
  refine ⟨X ^ q.den - C ((p : ℚᵘⁿ_[p]) ^ q.num *
    algebraMap ℤᵘⁿ_[p] ℚᵘⁿ_[p] (teichmuller p (c ^ q.den))),
  ...
  have htower : algebraMap ℚᵘⁿ_[p] 𝕃_[p] (algebraMap ℤᵘⁿ_[p] ℚᵘⁿ_[p]
      (teichmuller p (c ^ q.den))) = ZpUn_embd (teichmuller p (c ^ q.den)) := by
    change QpUn_embd (algebraMap ℤᵘⁿ_[p] ℚᵘⁿ_[p] _) = ZpUn_embd _
    unfold QpUn_embd
...
theorem shadow_mem_closure_integralClosure_of_support_int_div (n : ℕ+)
    ...
    shadow h ∈ closure (integralClosure ℚᵘⁿ_[p] 𝕃_[p]).carrier := by
  classical
  ...
  refine ⟨∑ q ∈ hfin.toFinset, single q (h.coeff q),
    Subalgebra.sum_mem _ fun q _ => single_mem_integralClosure_QpUn q _, ?_⟩
